<template>
  <div>
    <svg class="ma-1" version="1.1" viewBox="0 0 55.121 45.235" xmlns="http://www.w3.org/2000/svg">
      <g transform="translate(-68.53 -117.75)" stroke-width=".26458">
        <path
          id="inferior"
          class="inferior"
          :style="customStyle"
          @click="clickDente($event)"
          @mouseout="highlight('')"
          @mouseover="highlight(quadrante.inferior)"
        ...
        <path
          ...
          @click="clickDente($event)"
          @mouseout="highlight('')"
          @mouseover="highlight(quadrante.esquerda)"
          d="m74.018 153.79c-7.3117-7.8264-7.3181-18.887-0.0154-26.81 0.92313-1.0016 1.7916-1.8211 1.9299-1.8211 0.19131 0 11.355 8.2886 12.486 9.2704 0.12055 0.10465-0.11108 0.82983-0.51475 1.6115-1.2926 2.503-0.9194 6.1531 0.83507 8.1681 0.29688 0.34096-0.5711 1.1611-6.1604 5.8208-3.5783 2.9832-6.5871 5.4576-6.6862 5.4988-0.0991 0.0411-0.94242-0.7411-1.874-1.7383z"
        />
        <path
          id="direita"
          class="direita"
          @click="clickDente($event)"
          @mouseout="highlight('')"
          @mouseover="highlight(quadrante.direita)"
        ...
        <path
          ...
          class="centro"
          @click="clickDente($event)"
          @mouseover="highlight(quadrante.centro)"
          @mouseout="highlight('')"
        ...
        <path
          ...
          class="superior"
          @click="clickDente($event)"
          @mouseover="highlight(quadrante.superior)"
          @mouseout="highlight('')"
    ...
    >
      <template v-slot:activator="{ on }">
        <v-btn flat block :color="btnColor" v-on="on">
          <span>{{ notacao }}</span>
        </v-btn>
      </template>
      <v-card>
    ...
    <!-- <v-btn flat block :color="btnColor" @click="note">
      <span class="text-xs-center">{{ notacao }}</span>
    </v-btn> -->
    <!-- <p class="text-xs-center">{{notacao}}</p> -->
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    clickDente(item) {
      item.srcElement.classList.toggle("selected");
    },
    note() {
