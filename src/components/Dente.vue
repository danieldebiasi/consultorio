      ...
      <v-flex xs12 class="text-xs-center">
        ...
        >
          <g transform="translate(-68.53 -117.75)" stroke-width=".26458">
            <Divisao
              ...
              @divisaoClicked="showColorDialog"
        ...
        <v-dialog v-model="noteDialog" persistent max-width="400" transition="dialog-transition">
          ...
          <v-card>
            ...
            <v-textarea class="ma-3" name="anotacao" v-model="note" label="Anotação"></v-textarea>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn flat @click="dismissNote">Cancelar</v-btn>
              <v-btn flat @click="saveNote" color="success">Salvar</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>

        <v-dialog v-model="colorDialog" persistent max-width="400" transition="dialog-transition">
          <v-card>
            <v-card-title class="headline primary">
...
export default {
  ...
  data() {
    return {
      ...
      colorDialogDiv: "",
      colorDialogItems: {
        "nenhum": { title: "Nenhum", value: "nenhum" }, //cinza
        "feito": { title: "Feito", value: "feito" }, //azul
        "a-fazer": { title: "A fazer", value: "a-fazer" }, //vermelho
        "presente": { title: "Presente", value: "presente" } //verde
      },
      selectedColor: "nenhum",
      note: "",
      notacaoDente: this.notacao,
      quadranteDente: this.quadrante,
      clickedDivisao: "",
      status: {
        noteOriginal: "",
        mesial: "",
  ...
  methods: {
    dismissNote() {
      ...
      this.note = this.status.noteOriginal;
    },
    saveNote() {
      this.noteDialog = false;
      this.status.noteOriginal = this.note;
      this.btnColor = this.status.noteOriginal == "" ? "" : "info";
    },
    showColorDialog(item) {
      this.clickedDivisao = item;
      this.colorDialogDiv =
        item.srcElement.id.charAt(0).toUpperCase() +
        item.srcElement.id.slice(1);
      this.colorDialog = true;
    },
    selectColor(color) {
      this.clickedDivisao.srcElement.classList.remove(
        "feito",
        "a-fazer",
        "presente"
      );
      if (!this.clickedDivisao.srcElement.classList.contains(color)) {
        this.clickedDivisao.srcElement.classList.add(color);
      }
      this.$state.session.consulta.odontograma[this.notacaoDente][this.clickedDivisao.srcElement.id] = color;
      this.clickedDivisao = "";
  ...
  created() {
    ...
    if(this.$state.session.consulta.odontograma[this.notacaoDente] === undefined) {
      this.$state.session.consulta.odontograma[this.notacaoDente] = {anotacao: ''};
    }
  }
};
</script>
