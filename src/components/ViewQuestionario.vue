    ...
    <v-dialog v-model="dialog" persistent scrollable max-width="720px">
      ...
      <v-card>
        <v-card-title class="primary headline">
          ...
          <span class="white--text">Visualizar questionário</span>
          <v-spacer></v-spacer>
        ...
        <v-card-text>
          <v-layout row wrap>
            <v-flex xs6 pr-2>
              <v-text-field v-model="title" :disabled="!isEditing" label="Título"></v-text-field>
            </v-flex>
            <v-flex xs6 pl-2>
              <v-text-field v-model="category" :disabled="!isEditing" label="Categoria"></v-text-field>
            </v-flex>
          </v-layout>
        ...
        <v-card-actions>
          <v-tooltip right>
            <template v-slot:activator="{ on }">
              <v-btn :disabled="!isEditing" color="success" fab v-on="on" @click="addQuestion()">
                <v-icon>add</v-icon>
              </v-btn>
            </template>
          ...
          <v-btn color="success" :disabled="!isEditing" @click="create">Salvar</v-btn>
        </v-card-actions>
      </v-card>
